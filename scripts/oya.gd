...
func _fixed_process(delta):
	...
	var dash = Input.is_action_pressed("down")
	var stop = true
	...
	var chao = get_node("RayChao").is_colliding() || get_node("RayChao1").is_colliding() || get_node("RayChao2").is_colliding() || get_node("RayChao3").is_colliding()
	var andando =  walk_right || walk_left
	var rasteira = dash
	
	if andando:
		if velocity.x > 0:
			get_node("spriteOya").set_flip_h(false)
			
		else:
			get_node("spriteOya").set_flip_h(true)
